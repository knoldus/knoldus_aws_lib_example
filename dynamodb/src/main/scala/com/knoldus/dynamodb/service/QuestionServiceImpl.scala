...
class QuestionServiceImpl(questionTable: QuestionTable) extends QuestionService with LazyLogging{
  ...
  override def createQuestion(question: Question): Future[Option[String]] = Future {
    questionTable.put(question.record) match {
      case Right(id) =>
        logger.info("Question created successfully")
        Some(id)
      case Left(message) =>
        logger.error(s"Failed to create the question: $message")
        None
    }
  }

  override def getQuestion(id: String, category: String): Future[Option[Question]] = Future {
  ...
  override def updateQuestion(id: String, category: String, questionUpdate: QuestionUpdate): Future[Option[String]] = Future {
  ...
  override def deleteQuestion(id: String, category: String): Future[Unit] = Future {
